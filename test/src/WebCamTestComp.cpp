...
{
  WebCamTestInit(manager);
  WebCamInit(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("WebCamTest");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("WebCamTest0");
  if (comp == nullptr)
  {
  ...
  WebCamTest* testcomp = dynamic_cast<WebCamTest*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
